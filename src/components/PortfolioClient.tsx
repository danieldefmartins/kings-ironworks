...
export default function PortfolioClient({ initialCategory }: { initialCategory?: string }) {
  const router = useRouter();
  const pathname = usePathname();

  // Determine category from URL or prop
  ...
  const setCategory = useCallback((id: string) => {
    router.push(id === "all" ? "/portfolio" : `/portfolio/${id}`, { scroll: false });
  ...
  const handleImageClick = useCallback((photo: Photo) => {
    ...
    const idx = subPhotos.findIndex((p) => p.src === photo.src);
