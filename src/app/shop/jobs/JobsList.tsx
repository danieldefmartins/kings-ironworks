...
export default function JobsList({ jobs, lang, canSeeMoney, workingCount, progress }: { jobs: Job[]; lang: string; canSeeMoney: boolean; workingCount: Record<string, number>; progress: Progress }) {
  ...
  return <>
    ...
    {filtered.length === 0 ? <p className="py-16 text-center text-neutral-500">{t(lang, "noMatchingProjects")}</p> : <div className="grid gap-3 sm:grid-cols-2">{filtered.map((j) => { const p = progress[j.id] || { done: 0, total: 0 }; const pct = p.total ? Math.round((p.done / p.total) * 100) : 0; const d = days(j.due_date); return <Link key={j.id} href={`/shop/job/${j.id}`} className="block rounded-xl border border-neutral-800 bg-neutral-900 p-4 transition hover:border-amber-600/60 active:scale-[0.99]"><div className="flex items-start justify-between gap-2"><div className="min-w-0">{j.project_type && <div className="truncate text-[11px] font-bold uppercase tracking-wide text-amber-500">{j.project_type}</div>}<div className="truncate text-lg font-semibold">{j.customer_name}</div><div className="truncate text-xs text-neutral-500">{j.address || "—"}</div></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold text-white ${color(j.current_stage)}`}>{stageLabel(lang, j.current_stage)}</span></div>{workingCount[j.id] > 0 && <div className="mt-2 text-xs font-semibold text-green-400">● {workingCount[j.id]} {t(lang, "working")}</div>}{canSeeMoney && (contractValue(j) > 0 || depositValue(j) > 0) && <div className="mt-2 flex flex-wrap gap-2 text-xs font-bold"><span className="rounded-md border border-amber-700 bg-amber-950/60 px-2 py-0.5 text-amber-300">{t(lang, "contractPrice")} {money(contractValue(j))}</span>{depositValue(j) > 0 && <span className="rounded-md border border-emerald-700 bg-emerald-950/60 px-2 py-0.5 text-emerald-300">{t(lang, "depositPaid")} {money(depositValue(j))}</span>}</div>}<div className="mt-3 flex items-center justify-between text-xs"><span className={d < 0 ? "text-red-400" : d <= 7 ? "text-amber-400" : "text-neutral-400"}>{t(lang, "due")} {j.due_date || t(lang, "noDue")}</span><span className="text-neutral-400">{t(lang, "cutProgress")} {p.done}/{p.total}</span></div><div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-neutral-800"><div className="h-full bg-amber-500" style={{ width: `${pct}%` }} /></div></Link>; })}</div>}
  </>;
}
