...
export default function JobsList({ jobs, lang, canSeeMoney, workingCount, progress }: { jobs: Job[]; lang: string; canSeeMoney: boolean; workingCount: Record<string, number>; progress: Progress }) {
  ...
  return <>
    <div className="relative mb-4 flex min-w-0 gap-2">
      ...
      <button type="button" onClick={() => setControlsOpen((v) => !v)} aria-expanded={controlsOpen} aria-label={`${t(lang, "filter")} and ${t(lang, "sort")}`} className={`grid min-h-12 w-12 shrink-0 place-items-center rounded-xl border ${controlsOpen ? "border-amber-500 bg-amber-500/15 text-amber-300" : "border-neutral-700 bg-neutral-900 text-neutral-200"}`}><Filter className="h-5 w-5" /></button>
      {controlsOpen && <div className="absolute right-0 top-[calc(100%+8px)] z-20 w-[min(320px,calc(100vw-32px))] rounded-3xl border border-neutral-700 bg-neutral-950/95 p-4 shadow-2xl backdrop-blur-xl"><div className="mb-3 flex items-center justify-between"><span className="text-base font-semibold">{t(lang, "filter")} & {t(lang, "sort")}</span><button type="button" onClick={() => setControlsOpen(false)} aria-label={t(lang, "close")} className="grid h-8 w-8 place-items-center rounded-full bg-neutral-800"><X className="h-4 w-4" /></button></div><label className="mb-3 block"><span className="mb-1 block text-xs font-semibold uppercase tracking-wider text-neutral-500">{t(lang, "filter")}</span><select value={filter} onChange={(e) => setFilter(e.target.value)} className="min-h-12 w-full rounded-xl border border-neutral-700 bg-neutral-900 px-3 text-sm"><option value="all">{t(lang, "allProjects")}</option><option value="working">{t(lang, "currentlyWorking")}</option>{STAGES.map((s) => <option key={s} value={s}>{stageLabel(lang, s)}</option>)}</select></label><label className="block"><span className="mb-1 block text-xs font-semibold uppercase tracking-wider text-neutral-500">{t(lang, "sort")}</span><select value={sort} onChange={(e) => setSort(e.target.value)} className="min-h-12 w-full rounded-xl border border-neutral-700 bg-neutral-900 px-3 text-sm"><option value="due">{t(lang, "sortDue")}</option><option value="name">{t(lang, "sortName")}</option>{canSeeMoney && <option value="value">{t(lang, "sortValue")}</option>}</select></label></div>}
    </div>
