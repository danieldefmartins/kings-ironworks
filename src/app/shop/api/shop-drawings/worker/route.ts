...
import { sketchupPayload, type DrawingRequest } from '@/lib/shop/shop-drawings';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (body.action === 'claim') {
      ...
      try {
        const payload = sketchupPayload(row);
        const path = `${ORG_ID}/${row.id}/${row.lease_token}.skp`;
        return NextResponse.json({ request: { id: row.id, lease: row.lease_token, payload, uploadUrl: await drawingUploadUrl(path) } });
      } catch {
        await sbUpdate('kiw_shop_drawing_requests', `org_id=eq.${ORG_ID}&id=eq.${row.id}&lease_token=eq.${row.lease_token}`, { status: 'failed', error: 'Could not prepare SketchUp geometry or upload', lease_until: null });
        return bad('Could not prepare drawing', 422);
      }
    }
    const parsed = z.object({ action: z.enum(['complete', 'fail']), id: z.string().uuid(), lease: z.string().uuid(), error: z.string().max(500).optional() }).safeParse(body);
    ...
    const path = `${ORG_ID}/${b.id}/${b.lease}.skp`;
    if (b.action === 'complete' && !await drawingObjectExists(path)) return bad('Upload the SketchUp file first', 409);
    const rows = await sbUpdate<DrawingRequest[]>('kiw_shop_drawing_requests', filter, { status: b.action === 'complete' ? 'ready' : 'failed', artifact_path: b.action === 'complete' ? path : null, error: b.action === 'fail' ? b.error || 'SketchUp generation failed' : null, completed_at: new Date().toISOString(), lease_token: null, lease_until: null });
