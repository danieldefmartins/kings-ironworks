      ...
      {hasFlights && !isWallRail && <Card stage="specs" title={tx("Flight fabrication", "Fabricação por lance", "Fabricación por tramo")}>
        ...
        <details className="mt-3">
          ...
          <div className="space-y-4">
            ...
            <MInput label={tx("Bottom rail: vertical clearance above line through post bases", "Trilho inferior: folga vertical acima da linha das bases dos postes", "Riel inferior: separación vertical sobre línea de bases de postes")} value={data.fab.bottomClearance} onChange={v => set(d => { d.fab.bottomClearance=v; })} />
            <ChipRow label={tx("Bottom rail construction", "Montagem do trilho inferior", "Construcción del riel inferior")} value={data.fab.bottomRailConstruction ?? ""} options={[["between_posts", tx("Between plumb post faces", "Entre faces verticais dos postes", "Entre caras verticales de postes")], ["custom", tx("Custom", "Personalizado", "Personalizado")]]} onChange={v => set(d => { d.fab.bottomRailConstruction=v as NonNullable<MeasureData["fab"]["bottomRailConstruction"]>; })} />
            <MInput label={tx("Bottom rail gap at each post (horizontal)", "Folga em cada poste (horizontal)", "Holgura en cada poste (horizontal)")} value={data.fab.bottomRailEndGap ?? ""} onChange={v => set(d => { d.fab.bottomRailEndGap=v; })} />
            <ChipRow label={tx("Infill construction", "Montagem do preenchimento", "Construcción del relleno")} value={data.fab.infill} options={[["vertical pickets", tx("Vertical pickets between rail faces", "Balaústres verticais entre os trilhos", "Barrotes verticales entre rieles")], ["custom", tx("Custom", "Personalizado", "Personalizado")]]} onChange={v => set(d => { d.fab.infill=v; })} />
            <ChipRow label={tx("Picket spacing is measured as", "Espaçamento dos balaústres", "Medición de separación de barrotes")} value={data.fab.picketSpacingDatum ?? ""} options={[["max_clear_horizontal", tx("Maximum horizontal clear opening", "Vão livre horizontal máximo", "Abertura horizontal libre máxima")], ["custom", tx("Custom", "Personalizado", "Personalizado")]]} onChange={v => set(d => { d.fab.picketSpacingDatum=v as NonNullable<MeasureData["fab"]["picketSpacingDatum"]>; })} />
            <MInput label={tx("Picket gap at each rail (vertical)", "Folga em cada trilho (vertical)", "Holgura en cada riel (vertical)")} value={data.fab.picketEndGap ?? ""} onChange={v => set(d => { d.fab.picketEndGap=v; })} />
