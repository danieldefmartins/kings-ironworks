...
type Row = { id: string; status: string; requested_at: string; hasFile: boolean; error: string | null };
export default function ShopDrawingSubmission({ sheetId, lang, disabled, submit }: { sheetId: string; lang: string; disabled: boolean; submit: () => Promise<unknown> }) {
  ...
  const statuses: Record<string, string> = {
    queued: label('Queued for SketchUp', 'Na fila do SketchUp', 'En cola para SketchUp'),
    generating: label('SketchUp is generating', 'SketchUp está gerando', 'SketchUp está generando'),
  ...
  return <div className="space-y-3">
    <p className="text-sm text-neutral-300">{label('Send a saved measurement snapshot to SketchUp. The generated model needs shop review before fabrication.', 'Envie as medidas salvas ao SketchUp. O modelo gerado precisa de revisão antes da fabricação.', 'Envíe las medidas guardadas a SketchUp. El modelo generado requiere revisión antes de fabricar.')}</p>
    ...
    {rows.map(r => <div key={r.id} className="rounded-xl border border-neutral-700 p-3 text-sm"><p>{statuses[r.status] || r.status} · {new Date(r.requested_at).toLocaleString()}</p>{r.error && <p className="text-amber-300">{r.error}</p>}{r.hasFile && <a className="mt-2 inline-block underline text-sky-300" href={`/shop/api/shop-drawings?sheet=${sheetId}&download=${r.id}`}>{label('Download SketchUp draft (.skp)', 'Baixar rascunho SketchUp (.skp)', 'Descargar borrador SketchUp (.skp)')}</a>}</div>)}
  </div>;
}
