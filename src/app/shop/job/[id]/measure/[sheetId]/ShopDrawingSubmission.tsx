...
export default function ShopDrawingSubmission({ sheetId, lang, disabled, submit }: { sheetId: string; lang: string; disabled: boolean; submit: () => Promise<unknown> }) {
  ...
  const statuses: Record<string, string> = {
    queued: label('Queued for shop drawings', 'Na fila do Blender', 'En cola para Blender'),
    generating: label('Creating your drawing package', 'Blender está gerando', 'Blender está generando'),
  ...
  return <div className="space-y-3">
    <p className="text-sm text-neutral-300">{label('Create a custom KIW drawing package from your saved measurements. Supported railing geometry is modeled automatically; special assemblies and missing details are listed for shop detailing.', 'Crie desenhos KIW com as medidas salvas. Geometrias compatíveis são modeladas; peças especiais e detalhes pendentes ficam para revisão da oficina.', 'Cree planos KIW con las medidas guardadas. Las geometrías compatibles se modelan; piezas especiales y detalles pendientes requieren revisión del taller.')}</p>
