...
export function landingConnectionGeometry(data:MeasureData,t:LandingTransition){
  ...
  const endpoint=(id:string,segIdx:number,reach:string,sign:number):{point:Point3;post:PostMeasure;provisional:boolean}|null=>{
    ...
    return {post,point:{x:base.x+sign*extension*(end.x-start.x)/length,y:base.y+sign*extension*(end.y-start.y)/length,z:base.z+height+sign*extension*tread.rise/tread.run},provisional:model.provisional||data.datums.postRef!=='centerline'};
  };
