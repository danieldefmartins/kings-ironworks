...
if (!shopUrl || !output || !shopUrl.startsWith('https://')) throw new Error('Usage: node --env-file=.env.local scripts/blender/register-worker.mjs https://YOUR_SHOP /private/tmp/kiw-worker-config.json');
...
const response = await fetch(`${supabase}/rest/v1/kiw_shop_drawing_workers`, { method: 'POST', headers: { apikey: key, Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ org_id: org, name: 'Mac mini Blender', token_hash: createHash('sha256').update(token).digest('hex') }) });
